      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-sm">{record.fields.loc_name ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Typ der Location</Label>
            <Badge variant="secondary">{record.fields.loc_typ?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Verwendung</Label>
            <p className="text-sm">{Array.isArray(record.fields.loc_verwendung) ? record.fields.loc_verwendung.map((v: any) => v?.label ?? v).join(', ') : '—'}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm">{record.fields.loc_ansprechpartner_vorname ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Nachname des Ansprechpartners</Label>
            <p className="text-sm">{record.fields.loc_ansprechpartner_nachname ?? '—'}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm">{record.fields.loc_website ?? '—'}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Location ist verfügbar / bestätigt</Label>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
              record.fields.loc_verfuegbar ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
            }`}>
              {record.fields.loc_verfuegbar ? 'Ja' : 'Nein'}
            </span>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Datum der Besichtigung</Label>
            <p className="text-sm">{formatDate(record.fields.loc_besichtigung)}</p>
          </div>
          <div className="space-y-1">
